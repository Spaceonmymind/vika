...
abstract class AbstractSourceHandler implements OpenDataHandler
{
    ...
    public static function handle(array $data, BusinessSupportWidgetOdDataset $dataset): bool
    {
        ...
        try {
            ...
            Log::stack([static::DATASET_LOG_CHANEL, static::TELEGRAM_LOG_CHANEL])->error('od_handler_error Ошибка при обновлении мер поддержки предпринимателей',
                [
            ...
            Log::stack([ static::TELEGRAM_LOG_CHANEL])->error('od_handler_error Ошибка при обновлении мер поддержки предпринимателей - '.$dataset->url,
                );
            return false;
        }
        return true;
    ...
    abstract protected static function processData(array $data, BusinessSupportWidgetOdDataset $dataset);

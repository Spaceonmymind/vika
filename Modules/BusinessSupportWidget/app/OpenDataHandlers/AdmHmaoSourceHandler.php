...
class AdmHmaoSourceHandler extends AbstractSourceHandler
{
    ...
    protected const array FIELDS = [
        ...
        'VOZRAST_KOMPANII',
//        'NAIMENOVANIE_PARAMETRA_1', Исчезло из датасета 2025-10-10 и не используется у нас
//        'ZNACHENIE_PARAMETRA_1_MIN',
    ...
    public static function processData(array $data, BusinessSupportWidgetOdDataset $dataset): void
    {
        ...
        foreach ($rows as $row) {
            ...
            $subjectId = self::getReferenceId($subjects, static::prepareString($row['cols']['POLUCHATELI_PODDERZHKI']), BusinessSupportWidgetSubject::query());
            ...
            BusinessSupportWidgetMeasure::query()->create([
                ...
                'description' => static::prepareString($row['cols']['OPISANIE_PROGRAMMY']),
                ...
                'financial_support' => static::prepareString($row['cols']['RAZMER_FINANSOVOY_PODDERZHKI_I_VOZNAGRAZ']),
                'documents' => static::prepareString($row['cols']['PERECHEN_DOKUMENTOV_PREDOSTAVLYAEMYKH_Z']),
                'terms' => static::prepareString($row['cols']['SROKI_POLUCHENIYA_PODDERZHKI']),

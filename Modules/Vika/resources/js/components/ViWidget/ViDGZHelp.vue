  ...
  >
    ...
    >
      <div
        class="scroll-box"
      >
        <div>{{ detailQuestion.data.answer }}</div>
      </div>

      <template
        v-if="detailQuestion.data.link!==null"
        #footer
      >
        <div class="dialog-footer">
          <el-button
            ...
            style="width: 100%"
            type="primary"
            @click="setLink(detailQuestion.data.link)"
          >
            Подробно
          </el-button>
        </div>
    ...
    <div class="content-box">
      ...
      >
        ...
        <div
          ...
          @click="item.answer!==null ? setDetail(item) : item.link!==null ? setLink(item.link) : ''"
        >
...
export default{
  ...
  methods:{
    getCategories() {
      ...
      this.$axios.get(this.linkAPI + 'widget/gos_zakupki/get_categories')
        .then((response) => {
    ...
    getQuestions() {
      this.loader = true;
      this.$axios.get(this.linkAPI + 'widget/gos_zakupki/get_questions', {params: this.filter})
        .then((response) => {
    ...
    setDetail(activity) {
      this.detailQuestion.data = activity;

      ...
      >
        ...
        <div class="item-form">
          ...
          <el-select
            ...
            clearable
            :value-on-clear="null"
            @change="getCategories(); filter.category_id = null;"
          >
            <el-option
              v-for="item in servicesList"
              :key="'servicesList'+item.id"
              :label="item.name"
              :value="item.id"
            />
          </el-select>
...
export default {
  ...
  created() {
    ...
    Promise.all([this.getServices(), this.getCategories()]).finally(
      () => {
  ...
  methods: {
    async getCategories() {
      try {
        ...
        let params = {
          service_id: this.filter.service_id
        };
        let response = await this.$axios.get(this.linkAPI + 'widget/pfr_help/get_categories', {params});
    ...
    async getServices() {
      try {
        let response = await this.$axios.get(this.linkAPI + 'widget/pfr_help/get_services');
        console.log('Услуги: ', response.data);
    ...
    getQuestions() {
      this.loader = true;
      this.$axios.get(this.linkAPI + 'widget/pfr_help/get_questions', {params: this.filter})
        .then((response) => {
    ...
    startParams() {
      if (this.$route.query.service_id) {
        this.filter.service_id = parseInt(this.$route.query.service_id);
      }
